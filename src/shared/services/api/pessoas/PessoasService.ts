...
const getAll = async (
  ...
  try {
    ...
    console.error(error)
    return new Error((error as {message: string}).message || "Erro na listagem de registros");
  }
};
const getById = async (id: number): Promise<IDetalhePessoa | Error> => {
    try {
        ...
        console.error(error)
        return new Error((error as {message: string}).message || "Erro na consulta de registro");
      }
};
