...
const getAll = async (
  ...
  try {
    const urlRelativa = `${Environment.URL_BASE}/pessoas?_page=${page}&_limit=${Environment.LIMITE_DE_LINHAS}&nomeCompleto_like=${filter}`;
...
const create = async (
  ...
  try {
    const { data } = await Api.post<IDetalhePessoa>(`${Environment.URL_BASE}/pessoas`, dados);
