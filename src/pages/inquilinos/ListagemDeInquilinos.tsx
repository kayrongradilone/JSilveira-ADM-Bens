...
import {
  IListagemInquilinos,
 InquilinosService,
} from "../../shared/services/api/inquilinos/InquilinosService";
import { FerramentasDaListagem } from "../../shared/components";
import { LayoutBaseDePagina } from "../../shared/layouts";
import { useDebounce } from "../../shared/hooks";
import { Environment } from "../../shared/environment";

export const ListagemDeInquilinos: React.FC = () => {
  const [searchParams, setSearchParams] = useSearchParams();
  const { debounce } = useDebounce();
  const navigate = useNavigate();

  const [rows, setRows] = useState<IListagemInquilinos[]>([]);
            ...
            {rows.map((row) => (
              <TableRow key={row.id}>
                ...
                <TableCell sx={{ p: 1.4, mr: 1 }}>
                  ...
                  <IconButton
                    size="small"
                    onClick={() => navigate(`/inquilinos/detalhe/${row.id}`)}
                  >
                    <Icon>edit</Icon>
                  </IconButton>
                </TableCell>
